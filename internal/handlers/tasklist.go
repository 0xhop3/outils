...
func (h *TaskListHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	err = h.db.QueryRowContext(r.Context(),
		`INSERT INTO task_lists (user_id, name) VALUES ($1, $2)
		 RETURNING id, user_id, name, created_at`,
		userID, req.Name,
	).Scan(&taskList.ID, &taskList.UserID, &taskList.Name, &taskList.CreatedAt)
...
func (h *TaskListHandler) GetAll(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.QueryContext(r.Context(),
		`SELECT tl.id, tl.user_id, tl.name, tl.created_at
		 FROM task_lists tl
...
func (h *TaskListHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.db.ExecContext(r.Context(),
		`DELETE FROM task_lists tl
		 USING users u
		 WHERE tl.user_id = u.id AND tl.id = $1 AND u.firebase_uid = $2`,
		taskListID, firebaseUID,
	)
	if err != nil {
